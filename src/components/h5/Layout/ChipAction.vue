      ...
      <div class="chips-display" @click="handleChipClick">
        ...
        <div class="stacked-chip main-chip">
          <svg viewBox="0 0 78 78" class="chip" :class="`chip-${selectedChip}`">
            ...
            <text class="chip-value" :class="getChipValueClass(selectedChip)" x="50%" y="50%" font-weight="bold">{{ getChipText(selectedChip) }}</text>
          </svg>
        </div>
      </div>
...
.chip-value-large {
  font-size: 26px;  /* 一位数: 1-9 */
}

.chip-value-normal {
  font-size: 22px;  /* 两位数: 10-99 */
}

.chip-value-small {
  font-size: 18px;  /* 三位数: 100-999 */
}

.chip-value-small-more {
  font-size: 16px;  /* 四位数及以上: 1000+ */
}
